...
class humanoid_dh_long_Agent(baseAgent):
    ...
    def inference(self, obs_group):
        ...
        for name, obs in obs_group.items():
            obs_group[name] = torch.from_numpy(obs).to(self.device)
            obs_group[name].clamp_(-10., 10.)
        ...
        return dof_pos_target_urdf.cpu().numpy()
...
if __name__=="__main__":
    a=humanoid_dh_long_Agent("src/bxi_example_py_trunk/policy/model_xx.jit","cpu")
    obs_group={
        "dof_pos":np.zeros(12),
        "dof_vel":np.zeros(12),
    ...
    start = time.time()
    for i in range(100):
        print(a.inference(deepcopy(obs_group)))
    end = time.time()
