...
joint_kp = np.array([     # 指定关节的kp，和joint_name顺序一一对应
    ...
    100,100,100,150,30,10,
    50,20,10,20,10,
    50,20,10,20,10], dtype=np.float32)
...
left_hand_joint_index = [joint_name.index(name) for name in left_hand_joint_names]
right_hand_joint_index = [joint_name.index(name) for name in right_hand_joint_names]
upper_body_joint_index = left_hand_joint_index + right_hand_joint_index
left_in_upper = slice(0,5)
right_in_upper = slice(5,10)
left_hand_hold_pos = [0.0, 0., 0., -1.6, 0.0]
left_hand_send_pos = [-1.23, 0., 0., 0.0, 0.0]
right_hand_hold_pos = [0.0, 0., 0., -1.6, 0.0]
...
class BxiExample(Node):
    ...
    def __init__(self):
        ...
        self.left_btn_prev = False
        self.left_btn_changed = False
        self.right_btn_prev = False
        self.right_btn_changed = False
        self.stop_btn_prev = False
        self.stop_btn_changed = False
        self.upper_body_target_pos = joint_nominal_pos[upper_body_joint_index] # 上半身当前位置

        self.target_yaw = 0

        self.change_counter = None
        self.state = robotState.not_change
        self.left_state = leftHandState.stop
        self.right_state = rightHandState.stop


    def state_machine(self):
        "状态机"
        if self.state==robotState.not_change: # 当前没有正在切换
            if self.left_btn_changed:
                self.state = robotState.change
                # 左手按键在hold和send之间切换
                upper_body_current = self.upper_body_target_pos
                upper_body_target = upper_body_current.copy()

                if self.left_state == leftHandState.stop:
                    self.left_state = leftHandState.hold
                    print("left hand hold")
                    upper_body_target[left_in_upper] = left_hand_hold_pos
                elif self.left_state == leftHandState.hold:
                    self.left_state = leftHandState.send
                    print("left hand send")
                    upper_body_target[left_in_upper] = left_hand_send_pos
                elif self.left_state == leftHandState.send:
                    self.left_state = leftHandState.hold
                    print("left hand hold")
                    upper_body_target[left_in_upper] = left_hand_hold_pos

                self.change_counter = recoverCounter(2.0/self.loop_dt, upper_body_current, upper_body_target)

            if self.right_btn_changed:
                # 右手按键在hold和stop之间切换
                self.state = robotState.change
                upper_body_current = self.upper_body_target_pos
                upper_body_target = upper_body_current.copy()

                if self.right_state == rightHandState.stop:
                    self.right_state = rightHandState.hold
                    print("right hand hold")
                    upper_body_target[right_in_upper] = right_hand_hold_pos
                elif self.right_state == rightHandState.hold:
                    self.right_state = rightHandState.stop
                    print("right hand stop")
                    upper_body_target[right_in_upper] = joint_nominal_pos[right_hand_joint_index]
            ...
            if self.stop_btn_changed:
                ...
                print("stop all")
                upper_body_current = self.upper_body_target_pos
                upper_body_target = joint_nominal_pos[upper_body_joint_index]
                self.change_counter = recoverCounter(2.0/self.loop_dt, upper_body_current, upper_body_target)

        elif self.state==robotState.change:
            self.change_counter.step()
            if self.change_counter.finished:
                self.change_counter = None
                self.state = robotState.not_change
                print("stop change")
    ...
    def timer_callback(self):
        ...
        if self.step == 1:
            ...
            # 上半身
            if self.state == robotState.change:
                self.upper_body_target_pos = self.change_counter.current_dof_pos
                # np.set_printoptions(formatter={'float': '{:.2f}'.format})
                # print("target:",self.upper_body_target_pos)
            dof_pos_target[upper_body_joint_index] = self.upper_body_target_pos
    ...
    def joy_callback(self, msg):
        with self.lock_in:
            ...
            if self.step < 2:
                ...
                self.right_btn_prev = right_btn
                self.stop_btn_prev = stop_btn

            self.left_btn_changed = (left_btn != self.left_btn_prev)
            self.right_btn_changed = (right_btn != self.right_btn_prev)
            self.stop_btn_changed = (stop_btn != self.stop_btn_prev)
            # print("left:{},right:{},stop:{}".format(self.left_btn_changed, self.right_btn_changed, self.stop_btn_changed))
